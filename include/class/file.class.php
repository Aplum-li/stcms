...
$this->uploaded = $_SERVER['DOCUMENT_ROOT'].$this->save_path.$new_name;//上传后的文件名
...
//如果目录不存在，创建目录
	mkDirs($_SERVER['DOCUMENT_ROOT'].$this->save_path);
} 
} 
}
/************************************************* 
...
public function img_resized($w,$h,$source_img = NULL){ 
    ...
    $this->image_resize($this->source_img, $dst_name, $w, $h);
}
    public function set_dir($path){
            if(!is_dir($path)){
                if(!$this->set_dir(dirname($path))){
                    return false;
                }
                if(!mkdir($path,0777)){
                    return false;
